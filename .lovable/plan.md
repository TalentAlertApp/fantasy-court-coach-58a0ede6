...
## Plan: Replace venue image URLs with corrected NBA CDN sources

### Update `src/lib/nba-venues.ts`
Swap every `image` value in `NBA_VENUES` for the corrected URL from the newly uploaded `nba_venue_image_urls_v2-2.csv`. Arena `name` values stay unchanged.
...
### Files touched
- `src/lib/nba-venues.ts` — replace all 30 image URLs with the corrected NBA CDN URLs from the new CSV.

### Verification
- Open `/schedule`, scan multiple game cards: each row's background loads the correct official arena photo without broken images (e.g. CLE → Rocket Mortgage FieldHouse wide shot, LAL → Crypto.com Arena front shot, WAS → Capital One Arena wide shot).
- Network tab shows 200 OK for all `cdn.nba.com/manage/2024/08/*` requests.
