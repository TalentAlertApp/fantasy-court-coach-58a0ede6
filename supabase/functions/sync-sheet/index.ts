...
// deno-lint-ignore no-explicit-any
function rowToPlayerDbRow(row: string[]): any {
...
// deno-lint-ignore no-explicit-any
function rowToLastGame(row: string[]): any {
  const col = (i: number) => row[i] ?? "";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    let upsertedPlayers = 0, upsertedLastGames = 0;

    // Batch upsert players
    const playerRows = dataRows.map(rowToPlayerDbRow);
    for (let i = 0; i < playerRows.length; i += 50) {
      ...
      else console.error("Player upsert error:", error);
    }

    // Batch upsert last games
    const lgRows = dataRows.map(rowToLastGame).filter((lg: { game_date: string | null }) => lg.game_date);
    for (let i = 0; i < lgRows.length; i += 50) {
      const batch = lgRows.slice(i, i + 50);
      const { error } = await supabase.from("player_last_game").upsert(batch, { onConflict: "player_id" });
      if (!error) upsertedLastGames += batch.length;
      else console.error("Last game upsert error:", error);
    }

    return ok({ synced: true, players_upserted: upsertedPlayers, last_games_upserted: upsertedLastGames, total_rows: dataRows.length });
  } catch (e) {
