...
function TOTWCard({ player }: { player: TOTWPlayer }) {
  const teamLogo = getTeamLogo(player.team);
  const isFc = player.fc_bc === "FC";

  return (
    <div className="bg-card/95 backdrop-blur-sm border-t-2 rounded-lg overflow-hidden"
      ...
      <div className="flex justify-center py-0.5">
        {player.photo ? (
          <img src={player.photo} alt={player.name} className="w-12 h-12 rounded-full object-cover bg-muted" />
        ) : (
      ...
      <div className="border-t border-border/50 px-1 py-1 text-center">
        <span className="text-[9px] font-heading font-bold text-accent">{player.fp_avg} FP</span>
