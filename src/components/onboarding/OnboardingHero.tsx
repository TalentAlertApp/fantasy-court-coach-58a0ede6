...
export default function OnboardingHero({ onStart, onSignOut, onSkip, email }: Props) {
  return (
    <div className="relative flex flex-col h-screen overflow-hidden">
      ...
      {/* Hero content */}
      <main className="relative z-10 flex-1 min-h-0 flex flex-col items-center px-6 text-center pb-6">
        {/* Top: headline */}
        <div className="pt-4">
        ...
        {/* CTA cluster — docks at bottom, overlaps lower portion of marquee */}
        <div className="relative z-20 mt-auto flex flex-col items-center">
          ...
          <p className="mt-3 text-[11px] uppercase tracking-[0.3em] text-foreground/60">
            3 quick steps
          </p>
          ...
          <button
            type="button"
            onClick={onSkip}
            className="mt-2 text-[11px] uppercase tracking-[0.2em] text-foreground/50 hover:text-foreground/80 underline-offset-4 hover:underline transition-colors"
          >
            Skip for now →
          </button>
        </div>

        <div className="relative z-20 pt-6 flex flex-wrap items-center justify-center gap-3">
          {[
            "$100M Cap",
            "10 Players",
            "5 FC + 5 BC",
